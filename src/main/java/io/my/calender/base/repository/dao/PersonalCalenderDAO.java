...
public class PersonalCalenderDAO {
    ...
    public Flux<SearchPersonalCalenderListResponse> searchPersonalCalenderList(Long personalCalenderId, Long userId, Integer perPage, String title) {
        return this.personalCalenderQuery.searchPersonalCalenderList(personalCalenderId, userId, perPage, title)
                .map((row, rowMetadata) -> {
                    String imageUrl = row.get("user_image", String.class);
                    if (imageUrl != null) imageUrl = serverProperties.getImageUrl() + serverProperties.getImagePath() + imageUrl;
